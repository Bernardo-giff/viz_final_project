...
We cannott really get good insights from the boxplots as they are
because of the outliers. We will remove them to get a better look
...
We cannott really see a lot of difference between the distributions of
the cities. This is surprising, since both New York city and San
Francisco are known for their high prices. This might be do to the
...
ggplot(data = property_type_counts, aes(x = reorder(property_type, -count), y = count)) +
  geom_bar(stat = "identity", fill = airbnb_color1) +
  geom_line(aes(x = property_type, y = cumulative_percentage * max_count / 100), color = airbnb_color2) +
  geom_point(aes(x = property_type, y = cumulative_percentage * max_count / 100), color = airbnb_color2) +
...
types. Therefore, we will take the top 10 properties, which accounts for
over 90% of the data
...
ggplot(data=listings_filtered, aes(x= reorder(property_type, +n_properties), y=n_properties, fill=airbnb_color1))+
  ...
  geom_label(aes(label = n_properties, fill='white'))+
...
apartments in separate variables, and compare them row by row with the
expected monthly revenue of the airbnb listings. We define the revenue
p.a. as:

$$
Revenue_{p.m.} = \frac{(365 - Availability_{365 days})*Price_{per\ night}}{12}
$$ The the ROI % of an airbnb over the median rent is given by:

$$
ROI\ \% = \frac{Revenue_{p.m.} - median(Rent)}{median(Rent)}
$$
...
first to remove some evident outliers. We start by analyzing for the
entire dataset

```{r}

filtered_listings <- listings %>%
  filter(bedrooms == 1, price_night_median_rent <= 3)

ggplot(filtered_listings, aes(x = price_night_median_rent, fill = airbnb_color1)) +
  geom_density(alpha = 0.5) +  # Use geom_density for KDE
  labs(title = "Density of ROI %",
       x = "ROI %",
  ...
  theme(panel.grid.major = element_blank(),
        legend.position = "none",
        panel.grid = element_blank(),
        plot.background = element_rect(fill = background_color)) +  # Remove legend
  scale_x_continuous(labels = scales::percent_format(scale = 100))+
  scale_y_continuous(labels = scales::percent_format(scale = 100))

```

Then we also look at the distribution by city
...
We can't see any clear trend without any filters, but we apply a filter
for the number of days taken per year.

```{r}
filtered_listings <- listings %>%
  filter(bedrooms == 1, price_night_median_rent <= 3, availability_365<180)
  

ggplot(filtered_listings, aes(x = price_night_median_rent, fill = airbnb_color1)) +
  geom_density(alpha = 0.5) +  # Use geom_density for KDE
  labs(title = "Density of ROI % with more than 180 days taken",
       x = "ROI %",
       y = "Density") +  # Remove y-axis label
  theme_minimal() +
  theme(panel.grid.major = element_blank(),
...
## Price behavior in Austin

### Optimal number of minimum nights
Hosts could maximize their revenues if they knew the optimal minimum
nights. We aim to identify this optimal number by examining the
relationship between minimum nights, price and booking frequency. Our
goal is to provide valuable insights for potential Airbnb investors by
uncovering the most lucrative strategy for setting minimum night
requirements, considering additional factors like bed count for example.

There is no way to distinguish between rentals that have been blocked by
the host or haven been rented, for simplicity we will assume that
rentals that are not available, have been rented by users.

Let's first take a look at the 'listings_austin'. This dataset provides
...
```{r}
#Creating a copy of listings_austin
listings_austin_min <- listings_austin
# Quick overview of listings data
head(listings_austin_min)
str(listings_austin_min)
summary(listings_austin_min)
```

To wrap up our initial data exploration, let's list down all the columns
present in the listings_austin_min. This will give us a roadmap of the
variables we have at our disposal for the analysis.

```{r}
# Columns for listing data
column_names <- names(listings_austin_min)
print(column_names)
```
...
listings_austin_min$price <- as.numeric(gsub("[$,]", "", listings_austin_min$price))

#Convert Minimum Nights to integer
listings_austin_min$minimum_nights <- as.integer(listings_austin_min$minimum_nights)

#Convert Room type to Character
listings_austin_min$room_type <- as.character(listings_austin_min$room_type)

#Convert Availability 365 to integer
listings_austin_min$availability_365 <- as.integer(listings_austin_min$availability_365)

#Convert Number of Reviews to Integer
listings_austin_min$number_of_reviews <- as.integer(listings_austin_min$number_of_reviews)

#Convert Number of Reviews to Decimal (1 dp)
listings_austin_min$review_scores_rating <- as.numeric(listings_austin_min$review_scores_rating)
listings_austin_min$review_scores_rating <- round(listings_austin_min$review_scores_rating, 1)

#Convert Neighbourhood to character
listings_austin_min$neighbourhood <- as.character(listings_austin_min$neighbourhood)
```
...
# Price
na_price_listings <- sum(is.na(listings_austin_min$price))
cat("Number of NA values in Price (listings):", na_price_listings, "\n")

# Minimum nights
na_min_nights_listings <- sum(is.na(listings_austin_min$minimum_nights))
cat("Number of NA values in Minimum nights (listings):", na_min_nights_listings, "\n")

#Room Type
na_room_type_listings <- sum(is.na(listings_austin_min$room_type))
cat("Number of NA values in Room Type (listings):", na_room_type_listings, "\n")

#Availability 365
na_availability_365_listings <- sum(is.na(listings_austin_min$availability_365))
cat("Number of NA values in Availability 365 (listings):", na_availability_365_listings, "\n")

#Number of Reviews
na_number_of_reviews_listings <- sum(is.na(listings_austin_min$number_of_reviews))
cat("Number of NA values in Number of Reviews (listings):", na_number_of_reviews_listings, "\n")

# Reviews score rating
na_reviews_score_rating_listings <- sum(is.na(listings_austin_min$review_scores_rating))
cat("Number of NA values in Number of Reviews score rating (listings):", na_reviews_score_rating_listings, "\n")

#Neighbourhood
na_neighbourhood_listings <- sum(is.na(listings_austin_min$neighbourhood))
...
print(summary(listings_austin_min$price))

#Summary Stats for Minimum Nights
print(summary(listings_austin_min$minimum_nights))

#Summary Stats for Room Type
print(table(listings_austin_min$room_type))

#Summary Stats for Availability 365
print(summary(listings_austin_min$availability_365))

#Summary Stats for Number of Reviews
print(summary(listings_austin_min$number_of_reviews))

#Summary Stats for Review Score Rating
print(summary(listings_austin_min$review_scores_rating))

#Summary Stats for Neighbourhood
print(table(listings_austin_min$neighbourhood))
...
```{r}
#Price
boxplot(listings_austin_min$price, main="Boxplot of Prices", ylab="Price")
#to get a clearer picture, limiting it to 5000
boxplot(listings_austin_min$price, ylim=c(0,5000), main="Boxplot of Prices", ylab="Price")
#Exclude anything over 2000 (based on plot) and anything under (10 dollars) which seems to be too low
listings_austin_min <- listings_austin_min[listings_austin_min$price>=10 & listings_austin_min$price <= 800, ]
cat("Number of rows after filtering:", nrow(listings_austin_min), "\n")
```
...
```{r}
#Minimum Nights
boxplot(listings_austin_min$minimum_nights, main="Boxplot of Minimum Nights", ylab="Minimum Nights")
#to get a clearer picture, limiting it to 100
boxplot(listings_austin_min$minimum_nights, ylim=c(0,100), main="Boxplot of Minimum Nights", ylab="Minimum Nights")
#Exclude anything over 30 days based on plot
listings_austin_min <- listings_austin_min[listings_austin_min$minimum_nights <=30, ]
cat("Number of rows after filtering:", nrow(listings_austin_min), "\n")
```
...
```{r}
#Number of Reviews
boxplot(listings_austin_min$number_of_reviews, main="Boxplot of Number of Reviews ", ylab="Number of Reviews")
#Exclude anything above 600 based on plot
listings_austin_min <- listings_austin_min[listings_austin_min$number_of_reviews <=600, ]
cat("Number of rows after filtering:", nrow(listings_austin_min), "\n")
```
...
#Availability 365
boxplot(listings_austin_min$availability_365, main="Boxplot of Availability 365", ylab="Availability 365")

#Review Score Rating
boxplot(listings_austin_min$review_scores_rating, main="Boxplot of Review Score Rating", ylab="Review Score Rating")
```
...
listings_austin_min$booking_frequency <- (365 - listings_austin_min$availability_365)
#Creating booking frequency (%) to 1 dp
listings_austin_min$booking_frequency_percentage <- round(((365 - listings_austin_min$availability_365)/365)*100,1)
#Creating Revenue
listings_austin_min$estimated_revenue <- listings_austin_min$price * listings_austin_min$booking_frequency
```
...
adaptive_percent_format <- function (x) {
   ifelse (x == round (x),
           paste0(x, "%"),
...
library(ggplot2)
ggplot(listings_austin_min, aes(x=price)) +
  geom_density(aes(y=..count../sum(..count..)), fill= airbnb_color1, color= "black") +
  scale_x_continuous(labels=scales::dollar_format(prefix = "$"), limits=c(0,1000)) +
  scale_y_continuous(labels=adaptive_percent_format) +
  labs(title= "Density Plot of Price", x= "Price (USD)", y="Density") +
  theme_minimal() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
```

The bar chart visualizes the distribution of Airbnb listings based on
the minimum number of nights required for booking.

A significant majority of listings prefer short stays, with 1 and
2-night minimum stays each having just over 4000 listings. .3-night
minimum stays are also popular, nearing 1500 listings. .There's a sharp
...
ggplot(listings_austin_min, aes(x=as.factor(minimum_nights))) +
  geom_bar(fill= airbnb_color1) +
...
higher prices, there are specific minimum night where prices spike,
possibly influenced by limited listings.

```{r}
# Calculate weighted average
weighted_avg_price <- listings_austin_min %>%
...
ggplot(listings_austin_min, aes(x=minimum_nights, y=price)) +
...
```{r}
price_stats <- listings_austin_min %>%
  summarise(
...
#Correlation
correlation <- cor(listings_austin_min$minimum_nights, listings_austin_min$price, method= "pearson", use= "complete.obs")
print(correlation)
```
...
price_quantiles <- quantile(listings_austin_min$price, probs= c(0.25,0.5,0.75))

#Print the quantiles
print(price_quantiles)

#Plot a boxplot to visualise the distribution of Prices again
ggplot(listings_austin_min, aes(y= price)) +
  geom_boxplot() +
...
ggplot(listings_austin_min, aes(x= booking_frequency)) +
...
#Distribution of booking frequency %
ggplot(listings_austin_min, aes(x= booking_frequency_percentage)) +
  geom_histogram(binwidth = 10, fill = "skyblue", color ="black") +
...
booking_by_min_nights <- listings_austin_min %>%
  group_by(minimum_nights) %>%
  summarise(
...
ggplot(booking_by_min_nights, aes(x= minimum_nights, y= avg_booking_frequency_percentage)) +
  geom_bar(stat= "identity", fill = airbnb_color2) +
...
ggplot(listings_austin_min, aes(y= booking_frequency_percentage)) +
  geom_boxplot(fill= airbnb_color1) +
...
```{r}
correlation_bf_r <- cor(listings_austin_min$minimum_nights, listings_austin_min$booking_frequency_percentage, method= "pearson", use= "complete.obs")
print(correlation_bf_r)
```

The analysis highlights the variability in Airbnb pricing. To ensure
accurate comparisons, listings are segmented by the number of beds and
further categorized into pricing tiers using quartiles. This approach
offers a nuanced understanding, allowing for meaningful contrasts
between listings, regardless of their bed count. Most listings have 1
bed (around 5000), followed by 2 beds (around 3500) and 3 beds (around
2200). Listings drop sharply after 4 beds (around 1000) and continue to
decrease. Accommodations with many beds are rare.

```{r}
#Histogram of Number of Beds
ggplot(listings_austin_min, aes(x= beds))+
...
avg_price_by_beds <- listings_austin_min %>%
  group_by(beds) %>%
...
listings_austin_min <- listings_austin_min %>% filter(beds <= 10)

#Grouping the number of beds
listings_austin_min$beds_grouped <- case_when(
  listings_austin_min$beds == 1 ~ "1",
  listings_austin_min$beds == 2 ~ "2",
  listings_austin_min$beds == 3 ~ "3",
  listings_austin_min$beds == 4 ~ "4",
  listings_austin_min$beds == 5 ~ "5",
  listings_austin_min$beds >= 6 ~ "6+"
)
...
filtered_bed_data <- listings_austin_min %>% filter(beds <= 10)
...
avg_booking_by_beds <- listings_austin_min %>%
...
ggplot(listings_austin_min, aes(x=factor(beds_grouped), y= price)) +
  geom_boxplot() +
...
ggplot(listings_austin_min, aes(x= estimated_revenue, fill= factor(beds_grouped))) + 
...
ggplot(listings_austin_min, aes(x=factor(beds_grouped), y= estimated_revenue)) +
  geom_boxplot() +
...
mean_revenue <- mean(listings_austin_min$estimated_revenue, na.rm = TRUE)
median_revenue <- median(listings_austin_min$estimated_revenue, na.rm = TRUE)

# Plotting the histogram with enhanced colors and annotations
ggplot(listings_austin_min, aes(x = estimated_revenue)) +
  #geom_histogram(aes(y = ..density..), binwidth = 50, fill = "#66c2a5", color = "#238b45", alpha = 0.7) +
  geom_density(color = airbnb_color1, fill=airbnb_color1, alpha=0.2) +
  geom_vline(aes(xintercept = mean_revenue), color = airbnb_color2,linetype = "dashed", size = 1) +
  geom_vline(aes(xintercept = median_revenue), color = airbnb_color2, linetype = "dashed", size = 1) +
  labs(title = "Distribution of Estimated Revenues (Capped at 130K)",
       x = "Estimated Revenue in Thousands",
       y = "Density") +
  theme_minimal() +
  theme(panel.grid = element_blank()) + 
  annotate("text", x = mean_revenue, y = 0.00001, label = paste("Mean:", scales::dollar(round(mean_revenue, 2))), vjust = -1, color = "black") +
  annotate("text", x = median_revenue, y = 0.000005, label = paste("Median:", scales::dollar(round(median_revenue, 2))), vjust = -2, color = "black") +
...
listings_austin_min$dollar_revenue <- dollar(listings_austin_min$estimated_revenue)

# Calculate the correlation coefficient
correlation_value <- cor(listings_austin_min$minimum_nights, listings_austin_min$estimated_revenue)

# Create the scatter plot
ggplot(listings_austin_min, aes(x=minimum_nights, y=estimated_revenue)) +
  geom_point(alpha=0.5) +  # Adjust transparency
  geom_smooth(method="lm", color="blue", se=FALSE, formula = y ~ x) +  # Add regression line
  annotate("text", x = max(listings_austin_min$minimum_nights) - 5, 
           y = max(listings_austin_min$estimated_revenue), 
...
revenue potential. However, we can see that there are non-prime
neighborhoods which seem to be giving above average revenues. We believe
these should be great candidates for our revenue boosting strategy.

```{r}
# Calculate the average estimated_revenue for each zip code
avg_revenue_by_zip <- listings_austin_min %>%
...
listings_austin_min$Zip_Code <- as.character(listings_austin_min$neighbourhood_cleansed)

# Check for discrepancies in Zip Codes
discrepancies <- setdiff(listings_austin_min$Zip_Code, zip_map$Zip.Code)
if (length(discrepancies) > 0) {
  cat("These Zip Codes from listings_austin_min do not exist in zip_map:", paste(discrepancies, collapse = ", "))
} else {
  ...
  color_palette <- colorRampPalette(c("#66c2a5", "#d95f02", "#1f78b4", "#e7298a"))(10)
  # Create a custom color palette for mapping based on the ggplot color scheme
  color_palette <- colorRampPalette(c("#66c2a5", "#d95f02", "#1f78b4", "#e7298a"))(10)
...
The results below indicate that listings with a shorter minimum nights
requirement (1-7 nights) tend to generate higher revenue on average
compared to those with a longer minimum nights requirement (8+ nights).
This could be attributed to several factors, including higher demand for
short-term stays, flexibility for travelers, and potentially higher
nightly rates for short-term listings.

For hosts aiming to maximise their revenue, it might be beneficial to
consider setting their minimum nights requirement within the 1-7 nights
range. However, other factors such as location, amenities and guest
preferences should also be taken into account when making this decision.

```{r}
# Categorise the Minimum Nights 
listings_austin_min$min_nights_category <- ifelse(listings_austin_min$minimum_nights <= 7, "1-7 Nights", "8+ Nights")

# Group by Minimum Nights Category
grouped_data <- listings_austin_min %>%
...
We segmented the listings in our dataframe based on the number of beds,
categorizing them into groups such as "1", "2", "3", "4", "5", and "6+".
For each bed category, we further segmented the listings into pricing
quartiles, classifying them as "Budget", "Mid-Range", "Premium", and
"Luxury". Based on this segmentation, we make the assumption that then
"Luxury" listings, irrespective of bed count, are situated in prime
locations, offer superior amenities, and have consistently positive
reviews. The price point is indicative of the premium offerings these
listings provide. Conversely, the "Budget", "Mid-Range", and "'Premium"
listings offer amenities and experiences that align with their
respective price points. While this assumption provides a foundational
guideline for our analysis, individual listings may have characteristics
that deviate from these general categorizations.
...
listings_austin_min <- listings_austin_min %>%
  group_by(beds_grouped) %>%
...
The consistency in the results across all pricing categories reinforces
our earlier observation that listings with a shorter minimum nights
requirement tend to generate higher revenue.

This trend holds true even when accounting for different pricing
segments, suggesting that the minimum nights requirement plays a
significant role in influencing revenue, regardless of the listing's
price point.

Hosts across all pricing categories might consider setting their minimum
nights requirement within the 1-7 nights range to potentially maximise
their revenue. We will dive deeper into the data.

```{r}
# Organising Pricing Category
listings_austin_min$price_category <- factor(listings_austin_min$price_category, levels = c("Budget", "Mid-Range", "Premium", "Luxury"))
...
#Plot
ggplot(grouped_data, aes(x= min_nights_category, y= price_category, fill= weighted_avg_revenue)) +
  geom_tile() +
  geom_text(aes(label=scales::dollar_format()(weighted_avg_revenue)), color="black", size=3) +
  labs(title= "Heatmap of Weighted Average Revenue by Minimum Nights and Price Category", x="Minimum Nights Category", y= "Price Category") +
  theme_minimal() +
  scale_fill_gradient(low= "white", high = airbnb_color1,labels=scales::dollar_format(), name= "Weighted Average Revenue (USD)" ) +
  theme(axis.text.x = element_text(angle= 45, hjust= 1))+
  theme(legend.position ="none")
...
The higher standard deviation in the Luxury category than the other
categories indicates that our analysis for this segment might be less
robust compared to the other segments.

```{r}
#Distribution of Price by Number of Beds and Pricing Categories
price_stats_by_beds_and_category <- listings_austin_min %>%
...
Given the skewed distribution of our data, with a higher proportion of
Premium and Luxury listings in the 1-7 nights category, it's essential
to approach our analysis with caution. We cannot solely rely on the
aggregated insights from the 1-7 nights category. Instead, it's crucial
to delve deeper and examine the revenue metrics for each individual
minimum night within this category to draw more accurate conclusions.

```{r}
# The number of listings in each category combination
listings_count <- listings_austin_min %>%
  group_by(min_nights_category, price_category) %>%
  summarise(count= n())
print(listings_count)

# The proportions of listings in each category combination
listings_proportions <- listings_austin_min %>%
  group_by(min_nights_category) %>%
...
```{r}
booking_frequency_summary <- listings_austin_min %>%
...
ggplot(listings_austin_min, aes(x= price_category, y= booking_frequency_percentage, fill= price_category)) +
  geom_boxplot() +
...
The standard deviation in booking frequency is high most likely due to
the forward looking nature of the data.

```{r}
booking_frequency_by_min_nights <- listings_austin_min %>%
  group_by(min_nights_category,price_category) %>%
  summarise(
    mean_booking_frequency = mean(booking_frequency_percentage),
    median_booking_frequency= median(booking_frequency_percentage),
...
#Plotting the booking frequency for each pricing category
ggplot(listings_austin_min, aes(x= min_nights_category, y= booking_frequency_percentage, fill= price_category)) +
  geom_boxplot() +
...
filered_min_night_data <- listings_austin_min %>% filter(minimum_nights <= 30)
...
However, it is inaccurate to say that the optimal minimum night for
luxury is 9. Because there is only one luxury listing at 197,000.

```{r}
#Filter for Luxury Listings with minimum nights of 9
luxury_min_night_9 <- listings_austin_min %>%
...
Given the superior performance of the 1-7 nights category across all
segments and the limited data for the 8+ nights category that could lead
to inconclusive results, we recommend concentrating on the 1-7 nights
category for actionable insights. Analyzing this segment allows us to
make data-driven decisions based on where most of our business activity
occurs. While we acknowledge the presence of listings outside this
range, their relatively smaller proportion might introduce variability
that could skew our overall understanding. Therefore, our primary
analysis will center on the 1-7 nights range, ensuring that our insights
and recommendations are grounded in the most prevalent segment of our
data.

```{r}
#Histogram of Number of Beds
ggplot(listings_austin_min, aes(x= minimum_nights))+
  geom_histogram(binwidth = 1, fill= airbnb_color1, color= "black") +
  labs(title= "Distribution of Minimum Nights", x= "Minimum Nights", y= "Number of Listings") +
  theme_minimal() + 
  theme(panel.grid =element_blank())
```

For the luxury and premium price category, the optimal minimum night to
maximize revenue is 4; for mid-range, it's 5; and for budget, it's 3. We
believe that by following these insights, Airbnb hosts can enhance their
revenue potential.

```{r}
#Filter the data for 1-7 Minimum Nights
filered_min_night_data <- listings_austin_min %>% filter(minimum_nights <= 7)
...
#Determine the rows with the highest weighted average revenue for each price category
max_points <- optimal_nights_data %>%
  group_by(price_category) %>%
  filter(weighted_avg_revenue== max(weighted_avg_revenue)) %>%
  ungroup()
...
ggplot(optimal_nights_data, aes(x= as.factor(minimum_nights), y= weighted_avg_revenue, color= price_category, group= price_category)) +
  geom_line(size=1) +
  geom_point(size=3) +
  geom_point(data= max_points, aes(x= as.factor(minimum_nights), y=weighted_avg_revenue), color="black", size=4, shape=8) +
  labs(title= "Weighted Average Revenue by Minimum Nights and Price Category", x= "Minimum Nights", y= "Weighted Average Revenue (USD)", color= "Price Category")+
  theme_minimal() +
  theme(axis.text.x = element_text(angle= 45, hjust = 1)) +
  scale_y_continuous(labels = scales::dollar_format()) +
  scale_color_manual(values= c("Budget"=airbnb_color4, "Mid-Range"= airbnb_color2, "Premium"= airbnb_color3, "Luxury"= airbnb_color1))
```

## Entire apartment vs. private rooms
...
Assumptions for H3 - Listings that have 0 reviews and 0 availability for
the next 365 days will be removed from the data, and we will assume that
the host have blocked these listings. - Given the Airbnb data, there is
no way to distinguish between rentals that have been blocked by the host
or haven been rented, for simplicity we will assume that rentals that
are not available, have been rented by users.
...
USD350 per night and above USD50 per night. Finally, we have decided to
exclude Hotel Rooms (n=6) and shared room (n=98) because they have very
few listings.
...
p<-ggplot(data = austin_H3_2,aes(x = occupancy_rate, y = future_earnings)) + 
  ...
  theme(legend.position="none")  + scale_y_continuous(labels = dollar_format())
...
It can be seen that private rooms experience a higher occupancy rates
than entire home/apt. The majority of the data of the data for private
rooms is distributed along 60-100% of occupancy rates, while entire
home/apt is distributed along the mean (53.81%). It can be seen that our
Hypothesis 3 was not correct, the type of listings that enjoy higher
occupancy rates are the ones that are offered as private rooms rather
than Entire apartments. This insights could be transformed into
opportunity for investors interested in renting apartments in Austin
through Airbnb. Let's explore how higher occupancy rates are related to
revenue.
...
increase their revenue as a whole.

Around 30% of the Entire Apt listings offered in Austin have more than 1
room, and thus we could use that data points to understand the potential
revenue increase that a host could experience if they offered their
listings as private room rentals.
...
#Selecting entire apt listings that are earning less as entire apartments listings than they would if they offer their listing as a private room.
...
It can be seen from the graph that entire apartments that have 2 or more
bedroom can benefit from listings it as a private room.

The crucial insight here is that investors eyeing one-bedroom apartments should list them as entire apartments to maximize revenue, even though occupancy rates may be lower. Conversely, investors targeting multi-bedroom apartments should list them as private rooms to optimize both occupancy rates and revenue. By adopting this approach, they can expect to see an impressive average revenue increase of approximately 65% compared to offering the entire apartment as a single listing.
