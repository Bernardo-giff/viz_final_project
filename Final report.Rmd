...
ggplot(filtered_listings, aes(x = price_night_median_rent, fill = airbnb_color1)) +
  ...
  labs(title = "KDE of ROI %",
       x = "Price per Night / Median Rent",
...
ggplot(filtered_listings, aes(x = price_night_median_rent, fill = airbnb_color1)) +
  geom_density(alpha = 0.5) +  # Use geom_density for KDE
  labs(title = "KDE of ROI % with more than 180 taken",
       x = "Price per Night / Median Rent",
       y = NULL) +  # Remove y-axis label
  theme_minimal() +
  theme(panel.grid.major = element_blank(),
...
ggplot(listings_austin_min, aes(x=price)) + 
  geom_histogram(binwidth = 10, fill= airbnb_color1, color="black", alpha= 0.7) + 
...
ggplot(listings_austin_min, aes(y=price)) +
  geom_boxplot(fill= airbnb_color1, color= "black", alpha=0.7) +
  labs(title = "Boxplot of Prices for Listings in Austin", y="Price") +
  scale_y_continuous(labels = scales::dollar_format()) +
  theme_minimal() + 
  theme(panel.grid = element_blank(),
...
ggplot(listings_austin_min, aes(x=price, fill= as.factor(minimum_nights)))+
...
Median price for a single night is around 120. . Prices vary widely for
1-4 night stays, with many outliers. . 5-6 night stays show a reduced
price range, with a slight rise at 7 nights. . 9-night stays resemble
4-night stays in terms of price distribution. . Beyond 10 nights, for
the most part, prices stabilize with most listings priced between 100
and 200.

```{r}
# Another plot of Dstribution of Prices by Minimum Nights
ggplot(listings_austin_min, aes(x= as.factor(minimum_nights), y=price, fill= as.factor(minimum_nights))) +
  geom_boxplot() +
...
```{r}
listings_austin_min$min_nights_bin <- cut(listings_austin_min$minimum_nights,
                                    breaks= c(0,1,2,3,7,30),
                                    labels = c("1", "2", "3", "4-7", "8-30"),
                                    include.lowest = TRUE)

#Heatmap
ggplot(listings_austin_min, aes(x= as.numeric(min_nights_bin), y= price)) +
...
ggplot(listings_austin_min, aes(x= booking_frequency_percentage, y=price)) +
  geom_point(alpha=0.5) +
...
if (length(discrepancies) > 0) {
  ...
  color_palette <- colorRampPalette(c(airbnb_color1, airbnb_color2))(10)
...
Given that the majority of our listings are concentrated within the 1-7
minimum nights range, we believe that focusing our analysis on this
segment will provide the most representative and actionable insights.
Analyzing this segment allows us to make data-driven decisions based on
where most of our business activity occurs. While we acknowledge the
presence of listings outside this range, their relatively smaller
proportion might introduce variability that could skew our overall
understanding. Therefore, our primary analysis will center on the 1-7
nights range, ensuring that our insights and recommendations are
grounded in the most prevalent segment of our data.
...
We find that the optimal minimum night for price category of luxury and
premium is 4 (in terms of maximising revenue). For Mid-Range, it is 5
and for budget, it is 3.
